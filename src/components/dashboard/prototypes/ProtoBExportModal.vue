...
/**
 * PROTOTYPE B — Export Modal with tab bar
 * Tabs: Files (active) | Spotify (disabled)
 * Files tab: playlist selection + format/profile + mock export
 *
    ...
    <template v-if="tab === 'files'">
      <div v-if="phase === 'options'" class="proto-modal__body">
        ...
        <div class="proto-field">
          ...
          <div class="playlist-select-inline">
...
/* Tab bar */
.tab-bar { display: flex; gap: 0; border-bottom: 1px solid var(--color-border-subtle); }
.tab-bar__tab {
...
.tab-bar__tab--active { color: var(--color-text); border-bottom-color: var(--color-accent); }
...
/* Playlist selection inline */
.playlist-select-inline {
  display: flex;
